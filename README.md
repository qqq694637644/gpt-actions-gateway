...
The gateway only merges open, non-draft PRs whose head branch is still a `gpt/*` work branch. PR target/base branches are passed through to GitHub instead of being limited by `BASE_BRANCH_ALLOWLIST`.
