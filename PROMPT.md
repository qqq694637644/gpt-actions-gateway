...
## workspaceExecPwsh PowerShell 7 guidance and known pitfalls

`workspaceExecPwsh` runs **PowerShell 7 (`pwsh`) from the repository root**. Treat it as PowerShell, not Bash. The following rules are based on observed gateway behavior and should be used in every repository maintenance task.

### Recommended prelude for readable and UTF-8-safe output

For any non-trivial `workspaceExecPwsh` script, start with this prelude:
...
- `$PSStyle.OutputRendering = 'PlainText'` prevents ANSI color/control sequences from polluting tool output.
- UTF-8 output settings reduce the risk of mojibake when reading or writing Chinese, Markdown, JSON, logs, or source files.
- `$ProgressPreference = 'SilentlyContinue'` avoids noisy progress records in captured stdout.
- `$ErrorActionPreference = 'Stop'` makes failures explicit instead of silently continuing after partial execution.

### Known issues observed with workspace PowerShell

1. **ANSI control sequences in table output**

   Symptom: `Format-Table` or formatted object output may include sequences like `\x1b[32;1m` when rendered by the assistant UI.

   Mitigation:

   - Prefer plain objects or explicit string formatting over `Format-Table` when output will be inspected by the model.
   - Use the standard prelude with `$PSStyle.OutputRendering = 'PlainText'`.
   - If table output is still noisy, use `Select-Object` plus `ForEach-Object` string rendering.
...
2. **PowerShell is not Bash; Bash heredoc syntax fails**
   ...
   In PowerShell, use a here-string and pipe it to the process:
...
3. **CRLF/LF Git warnings on Windows workspaces**

   Symptom: Git may print warnings such as `LF will be replaced by CRLF the next time Git touches it`.

   This is not content corruption, but it is noisy and can hide real warnings. Mitigate by:

   - Prefer `workspaceWriteFile(..., line_ending='lf')` for new full-file writes when the repository uses LF.
   - Prefer repository `.gitattributes` for stable line endings, for example:
   ...
   - Do not treat CRLF warnings as validation failures unless `git diff --check` or tests fail.

4. **Encoding-sensitive file reads and writes**

   - Prefer `workspaceWriteFile` for full UTF-8 text replacements because it enforces UTF-8 and supports line-ending control.
   - In PowerShell, specify `-Encoding utf8` when using `Set-Content`, `Add-Content`, or `Out-File`.
   - Avoid Windows PowerShell assumptions; the workspace shell is `pwsh`, but explicit UTF-8 is still safer.

5. **Formatted command output may be less stable than structured Gateway tools**

   Continue to prefer structured Gateway tools for Git/PR/CI state:

   - Use `workspaceStatus` instead of parsing `git status`.
   - Use `workspaceDiff` instead of parsing `git diff` for final review.
   - Use `workspaceCommitAndPush` instead of manual commit/publish commands.
   - Use `queryCiStatus`, `getJobLog`, and related CI tools instead of ad-hoc GitHub CLI commands.
...
```powershell
# List top-level repository files without ANSI-heavy formatting.
Get-ChildItem -Force |
  Sort-Object Name |
  Select-Object Mode, Length, Name |
  ForEach-Object { "{0} {1,10} {2}" -f $_.Mode, $_.Length, $_.Name }

# Search text while excluding generated/dependency/cache directories.
Get-ChildItem -Recurse -File |
  Where-Object { $_.FullName -notmatch '\\.git\\|node_modules|dist|build|coverage|__pycache__|\.venv|\.pytest_cache' } |
  Select-String -Pattern '<keyword>' -Context 2,2

# Feed a multi-line Python snippet to Python from pwsh.
$code = @'
...
### Backend improvement suggestions to mention when relevant

If the user asks about gateway quality or wants backend hardening, report these concrete improvements:

- Inject the recommended PowerShell prelude automatically in `workspaceExecPwsh` or provide an opt-in `plain_output` mode.
- Strip ANSI escape sequences from captured stdout/stderr before returning tool output, or force `$PSStyle.OutputRendering='PlainText'` at process startup.
- Start pwsh with UTF-8-friendly environment variables and console encodings.
- Configure workspace Git line endings consistently, or document that repositories should use `.gitattributes`.
- Add tests that execute `workspaceExecPwsh` scripts emitting Chinese text, ANSI-colored output, and CRLF/LF-sensitive files.

Do not overstate these observations: during recent repository maintenance, UTF-8 file writing and Chinese Markdown content worked correctly; the main observed issues were ANSI noise, PowerShell-vs-Bash heredoc syntax, Git line-ending warnings, and security scanning that may reject forbidden command strings even when they appear in explanatory text.
